...
while(true){
	...
	$id = explode("'",explode('https://'.host.'/ptc/view/',$r)[1])[0];
	if($id){
		ulang:
		$r = Curl('https://'.host.'/ptc/view/'.$id,h())[1];
		$code = explode('</span>',explode('<span class="text-color text-capitalize">',$r)[1])[0];
		$gp = Gp($code);
		if($gp[1]){
			...
			$r1 = Curl('https://'.host.'/ptc/verify/'.$id,h(),$data)[1];
			$ss = explode('has',explode("text: '",$r1)[1])[0];
			if($ss){
				echo b."Visit : ".k.$ad.n;
				echo h."Success    ~> ".k.$ss.n;
...
wd:
$r = Curl('https://'.host.'/withdraw',h())[1];
...
$r2 = Curl('https://'.host.'/withdraw/withdraw',h())[1];
